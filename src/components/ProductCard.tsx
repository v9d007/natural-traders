...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full h-48 sm:h-52 bg-slate-50/90 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800 overflow-hidden">
        ...
        {hasMultipleImages && (
          <>
            ...
            <button
              ...
              className="absolute right-2 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-white/90 dark:bg-slate-900/90 hover:bg-white text-slate-700 dark:text-slate-200 shadow-md flex items-center justify-center backdrop-blur-xs opacity-0 group-hover:opacity-100 transition-all duration-200 z-10 hover:scale-110 border border-slate-200/60 dark:border-slate-700"
            >
              <ChevronRight className="w-4 h-4" />
            </button>

            {/* Subtle Carousel Dots Indicator */}
            <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex items-center gap-1 z-10 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-200">
              {imagesList.map((_, idx) => (
                <span
                  key={idx}
                  className={`h-1.5 rounded-full transition-all duration-200 ${
                    currentImgIndex === idx
                      ? "w-3.5 bg-emerald-600 shadow-xs"
      ...
      <div className="p-4 flex-1 flex flex-col justify-between space-y-3">
        <div className="space-y-2.5">
          ...
          <Link href={`/products/${product.slug}`} className="block group/title">
            <h3 className="text-sm sm:text-base font-bold text-slate-900 dark:text-white leading-snug group-hover/title:text-emerald-600 dark:group-hover/title:text-emerald-400 transition-colors line-clamp-2 min-h-[2.75rem]">
              {displayName}
            </h3>
          </Link>

          {/* Clean Specs List - Generous vertical space, no clipped text */}
          <div className="space-y-2 text-xs bg-slate-50 dark:bg-slate-800/50 p-2.5 rounded-xl border border-slate-100 dark:border-slate-800/80">
            <div className="flex items-start justify-between gap-2">
              <span className="text-slate-500 dark:text-slate-400 text-[11px] font-medium flex items-center gap-1.5 shrink-0 pt-0.5">
                <Package className="w-3.5 h-3.5 text-slate-400" />
                <span>{t.unitPackaging}:</span>
              </span>
              <span className="font-semibold text-slate-800 dark:text-slate-200 text-right text-[11px] leading-tight">
                {displayPack}
              </span>
            </div>

            <div className="flex items-center justify-between gap-2 pt-1.5 border-t border-slate-200/60 dark:border-slate-700/60">
              <span className="text-emerald-700 dark:text-emerald-400 text-[11px] font-medium flex items-center gap-1.5 shrink-0">
                <Layers className="w-3.5 h-3.5 text-emerald-500" />
                <span>{t.moqLabel}:</span>
              </span>
              <span className="font-bold text-emerald-700 dark:text-emerald-400 text-right text-[11px]">
                {displayMoq}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 text-xs pt-1">
            <span className="text-slate-400 dark:text-slate-500 text-[11px]">MRP:</span>
        ...
        <div className="pt-2.5 border-t border-slate-100 dark:border-slate-800">
          <Link
            href={`/products/${product.slug}`}
            className="w-full flex items-center justify-center gap-2 h-10 px-4 rounded-xl bg-slate-900 hover:bg-slate-800 dark:bg-emerald-600 dark:hover:bg-emerald-700 active:scale-[0.98] text-white text-xs sm:text-sm font-bold transition-all shadow-sm"
          >
